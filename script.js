        ...
        filmElement.innerHTML = `
            ...
            <div class="rating">
                <div class="stars" data-film-id="${film.id}">
                    ${generateStars(film.id, film.rating, film.userRated)}
                </div>
                <div class="rating-info">
                    <span>Рейтинг: <strong>${film.rating.toFixed(1)}</strong>/5.0</span>
                    <span>Оценок: ${film.votes}</span>
                    ${film.userRated ? '<span class="user-rated">✓ Вы оценили</span>' : ''}
                </div>
            </div>
        `;
...
function generateStars(filmId, currentRating, userRated) {
    let starsHTML = '';
    for (let i = 1; i <= 5; i++) {
        const isActive = i <= Math.round(currentRating);
        const canRate = !userRated ? 'clickable' : 'disabled';
        starsHTML += `<i class="fas fa-star star ${isActive ? 'active' : ''} ${canRate}" data-value="${i}"></i>`;
    }
    return starsHTML;
}
...
function rateFilm(filmId, rating) {
    console.log(`⭐ Пользователь оценивает фильм ID ${filmId}: ${rating} звезд`);
    ...
    // Проверяем, оценивал ли уже пользователь этот фильм
    if (film.userRated) {
        alert('⚠️ Вы уже оценили этот фильм! Оценку можно поставить только один раз.');
        console.warn(`Пользователь пытается повторно оценить фильм "${film.title}"`);
        return;
    }
    ...
    film.userRated = true; // Отмечаем, что пользователь оценил
    
    console.log(`✅ Новый рейтинг "${film.title}": ${film.rating.toFixed(1)} (оценок: ${film.votes})`);
    
    // Сохраняем в localStorage, что пользователь оценил этот фильм
...
function saveUserRating(filmId) {
    try {
        const ratedFilms = JSON.parse(localStorage.getItem('user-rated-films')) || [];
        if (!ratedFilms.includes(filmId)) {
            ratedFilms.push(filmId);
            localStorage.setItem('user-rated-films', JSON.stringify(ratedFilms));
        }
    } catch (e) {
        console.error('❌ Ошибка сохранения оценки пользователя:', e);
    }
}
...
function loadUserRatings() {
    try {
        ...
        console.error('❌ Ошибка загрузки оценок пользователя:', e);
    }
}
...
// Добавляем стили для уведомлений и disabled звёзд
